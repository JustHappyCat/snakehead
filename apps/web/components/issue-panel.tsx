...
function getWhyItMatters(issueType: string): string {
  const whyMap: Record<string, string> = {
    ...
    BROKEN_EXTERNAL_LINK: 'Broken external links frustrate users and can signal outdated or unmaintained content.',
    INTERNAL_LINK_TO_REDIRECT: 'Internal links that hit redirects add latency and dilute crawl efficiency.',
    DUPLICATE_CONTENT: 'Duplicate or near-duplicate content splits ranking signals across pages and can suppress all versions in search results.',
    ...
    POOR_LCP: 'Slow Largest Contentful Paint frustrates visitors and is a direct Google ranking factor via Core Web Vitals.',
    POOR_CLS: 'Layout shifts cause misclicks and a janky experience, and hurt Core Web Vitals assessment.',
    POOR_INP: 'Slow interaction response makes the page feel broken and fails the INP Core Web Vital.',
    LOW_PERFORMANCE_SCORE: 'A low Lighthouse score indicates compounding speed problems that hurt both users and rankings.',
...
function getWhatItMeans(issueType: string): string {
  const whatMap: Record<string, string> = {
    ...
    BROKEN_EXTERNAL_LINK: 'This page links to one or more external URLs that return an error or could not be reached.',
    INTERNAL_LINK_TO_REDIRECT: 'This page links to internal URLs that redirect before loading.',
    DUPLICATE_CONTENT: 'This page&apos;s main content is identical or highly similar to at least one other crawled page.',
    ...
    POOR_LCP: 'The largest visible element on this page takes longer than Google&apos;s 2.5s target to render.',
    POOR_CLS: 'Content on this page visibly shifts around while loading, beyond Google&apos;s 0.1 CLS target.',
    POOR_INP: 'Real users experience delayed responses to taps and clicks on this page, beyond the 200ms INP target.',
    LOW_PERFORMANCE_SCORE: 'This page scored below 50/100 in a Lighthouse performance test.',
